...
public class CarSort extends Display {

    protected ArrayList<Car> list = new ArrayList<Car>();

    public CarSort(String fileName) throws FileNotFoundException {
        super(fileName);
    ...
    public int partition(ArrayList<Car> list, int start, int end) {
        ...
        for (int x = start + 1; x <= end; x++) {
            if (list.get(x).compareToYear(pivotCar) < 0) {
                endOfList++;
                Collections.swap(list, x, endOfList);
            }
            else if (list.get(x).compareToYear(pivotCar) == 0 && list.get(x).compareToVIN(pivotCar) <= 0) {
                endOfList++;
                Collections.swap(list, x, endOfList);
            }
        }
